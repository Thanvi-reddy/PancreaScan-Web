...
function App() {
  ...
  useEffect(() => {
    try {
      // Clear any previous simulation/static data to guarantee 100% strictly dynamic data
      localStorage.removeItem('simulated_scans');
  ...
  const fetchScansHistory = async (userEmail: string) => {
    ...
    try {
      ...
      showToast(`⚠️ Sync Database Error: Failed to load scans from server. (${err.message || 'API unavailable'})`);
      setScans([]);
      calculateAnalytics([]);
    } finally {
      setLoading(false);
  ...
  const callAuthApi = async (formData: FormData): Promise<any> => {
    try {
      ...
      showToast(`❌ Connection Error: Failed to reach backend auth server. (${err.message || 'API unreachable'})`);
      return null;
    }
  };
